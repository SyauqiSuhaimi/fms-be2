"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.HolidayService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const company_service_1 = require("../company/company.service");
const holiday_entity_1 = require("../entities/holiday.entity");
const typeorm_2 = require("typeorm");
const xlsx = require("xlsx");
const fs = require("fs");
const general_service_1 = require("../helper/general.service");
let HolidayService = class HolidayService {
    constructor(HolidayRepository, CompanyService, GeneralService) {
        this.HolidayRepository = HolidayRepository;
        this.CompanyService = CompanyService;
        this.GeneralService = GeneralService;
    }
    findAll(condition) {
        return this.HolidayRepository.find(condition);
    }
    async findOne(condition) {
        return this.HolidayRepository.findOne(condition);
    }
    create(itemData) {
        return this.HolidayRepository.save(itemData);
    }
    async update(id, itemData) {
        return this.HolidayRepository.update(id, itemData);
    }
    async remove(id) {
        return this.HolidayRepository.delete(id);
    }
    async byCompany(userData) {
        return this.HolidayRepository.find({
            where: { company: { user: { id: userData["id"] } } },
        });
    }
    async generateHoliday(userData, post_input, getYear) {
        let company = await this.CompanyService.findCompany({
            where: { user: { id: userData["id"] } },
        });
        let holidays = [];
        console.log("year", getYear);
        for (let i = 0; i < 365; i++) {
            let date = new Date(getYear, 0, i);
            let day = date.getDay();
            let result = post_input.indexOf("Friday - Saturday");
            if (result != -1) {
                if (day === 6 || day === 5) {
                    holidays.push({
                        name: day === 6 ? "Saturday" : "Friday",
                        date: (date.getTime() / 1000).toString(),
                    });
                }
            }
            else {
                if (day === 6 || day === 0) {
                    holidays.push({
                        name: day === 6 ? "Saturday" : "Sunday",
                        date: (date.getTime() / 1000).toString(),
                    });
                }
            }
        }
        console.log("Company", company);
        return holidays;
    }
    async parseExcelFile(files, company) {
        if (!files || files.length === 0) {
            console.error("No files passed to parseExcelFile function");
            return;
        }
        const file = files[0].path;
        console.log(file, "sini tgk apa keluar");
        const workbook = xlsx.read(fs.readFileSync(file).toString("binary"), {
            type: "binary",
        });
        const worksheet = workbook.Sheets[workbook.SheetNames[0]];
        const data = xlsx.utils.sheet_to_json(worksheet);
        console.log(data, "sini nak nk tengok data yg dah read");
        const processedData = this.preProcessData(data, company);
        await this.saveData(processedData);
        console.log(processedData, "sini tgk data dia save ke x?");
    }
    preProcessData(data, company) {
        try {
            return data.map((item) => ({
                name: item.name,
                description: item.description,
                date: this.GeneralService.strToEpoch(item.date),
                company: item.company || company,
            }));
        }
        catch (error) {
            console.error(error);
        }
    }
    async saveData(processedData) {
        for (const item of processedData) {
            const holiday = this.HolidayRepository.create(item);
            await this.HolidayRepository.save(holiday);
        }
    }
};
HolidayService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(holiday_entity_1.Holiday)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        company_service_1.CompanyService,
        general_service_1.GeneralService])
], HolidayService);
exports.HolidayService = HolidayService;
//# sourceMappingURL=holiday.service.js.map