...
let qcController = class qcController {
    constructor(qcService, jwtService, GeneralService) {
        this.qcService = qcService;
        this.jwtService = jwtService;
        this.GeneralService = GeneralService;
    }
    async fillAll() {
        const response = await this.qcService.findAll();
        return response;
    }
    async findsubtask(id, request) {
        ...
        if (data["usertype"] == "superadmin") {
            response = await this.qcService.find({
                where: { company: { id: id } },
            });
        }
        else {
    ...
    async findOne(id) {
        const response = await this.qcService.findOne({ where: { id: id } });
        if (!response) {
    ...
    async create(createqcData) {
        const response = await this.qcService.create(createqcData);
        return response;
    }
    async update(id, updateqcData) {
        const response = await this.qcService.findOne({ where: { id: id } });
        if (!response) {
            throw new common_1.BadRequestException("No Data");
        }
        else {
            const response2 = await this.qcService.update(id, updateqcData);
            return updateqcData;
        }
    }
    async delete(id) {
        ...
        if (!response) {
            ...
            const response = await this.qcService.remove(id);
            return response;
        }
    }
    async uploadFile(body, files, request) {
        ...
        this.qcService.parseExcelFile(files, company);
    }
    async downloadExcelData(res, request) {
        let cookie = request.cookies["jwt"];
        const data = await this.jwtService.verifyAsync(cookie);
        const response = await this.qcService.find({
            where: { company: { user: { id: data["id"] } } },
            relations: ["company"],
        });
        if (!response) {
...
], qcController.prototype, "fillAll", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], qcController.prototype, "findsubtask", null);
__decorate([
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [qc_entity_1.qc]),
    __metadata("design:returntype", Promise)
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, qc_entity_1.qc]),
    __metadata("design:returntype", Promise)
...
__decorate([
    ...
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], qcController.prototype, "delete", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], qcController.prototype, "uploadFile", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], qcController.prototype, "downloadExcelData", null);
__decorate([
    ...
    __metadata("design:returntype", Promise)
], qcController.prototype, "downloadexcel", null);
qcController = __decorate([
